...
* решение задачи №4 находится [здесь](TestClassMemory.php). Использование памяти происходило волнообразно. Плавно увеличивается до определенного значения, потом резкий спад использованного объёма. Это обусловнлено работой сборщика мусора. Он вычисляет циклические ссылки `$newObj->self = $newObj;` и делает очистку памяти от таких объектов. Сам сборщик мусора запускается автоматически при достижении значения GC_ROOT_BUFFER_MAX_ENTRIES d 10 000 штук по умолчанию. В **php 7.3.3** сборщик срабатывал на 2657208 байтах использования памяти.
